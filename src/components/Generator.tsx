...
export default () => {
  ...
  const shouldAutoScroll = () => {
    if (!messagesContainerRef) return false;
    
    const threshold = 100; // 距离底部的像素阈值
    const isNearBottom = messagesContainerRef.scrollHeight - messagesContainerRef.scrollTop - messagesContainerRef.clientHeight < threshold;
    
    return isNearBottom;
  };

  // 优化后的滚动函数
  const scrollToBottom = _.throttle(function () {
    if (messagesContainerRef && shouldAutoScroll()) {
  ...
  return (
    <div my-6>
      ...
      <div 
        ...
        class="max-h-[70vh] overflow-y-auto my-4 px-2 pb-4"
      >
