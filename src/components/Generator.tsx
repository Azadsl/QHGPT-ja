...
export default () => {
  ...
  const [isFirstStreamUpdate, setIsFirstStreamUpdate] = createSignal(true);
  ...
  onMount(async () => {
    ...
    // Set up scroll detection - track user scroll behavior
    const handleScroll = _.throttle(() => {
      const currentScrollPosition = window.scrollY;
      const maxScroll = document.body.scrollHeight - window.innerHeight;
      
      // If we're scrolling up, mark that the user has manually scrolled
      if (currentScrollPosition < lastScrollPosition()) {
        setUserScrolledUp(true);
      }
      
      // If we're at the bottom, reset the userScrolledUp flag
      if (maxScroll - currentScrollPosition < 50) {
  ...
  // Modified effect to handle streaming updates without forcing scrolls
  createEffect(() => {
    // Only trigger for changes to the assistant message
    const message = currentAssistantMessage();
    
    // Don't do anything if there's no message
    if (!message || !messagesEndRef) return;
    
    // Only scroll if user hasn't scrolled up manually
    // And avoid scrolling during streaming updates after the first one
    if (!userScrolledUp() && isFirstStreamUpdate()) {
      setIsFirstStreamUpdate(false);
      // Use a very slight delay to allow the content to render first
      setTimeout(() => {
  ...
  const handleButtonClick = async () => {
    ...
    // When user sends message, we'll scroll to bottom
    setUserScrolledUp(false);
    
    // Scroll to bottom before starting stream
    if (messagesEndRef) {
