...
import { createMemo, createEffect, JSX } from 'solid-js';
...
export default ({ role, message, assistantAvatar, showRetry, onRetry }: Props) => {
  ...
  const [messageEl, setMessageEl] = createSignal(null);
  createEffect(() => {
    if (messageEl()) {
    ...
    <div class="py-2 -mx-4 px-4 transition-colors md:hover:bg-slate/3">
      <div class="flex gap-3 rounded-lg" class:op-75={role === 'user'}>
        <div class={`shrink-0 w-7 h-7 mt-4 rounded-full op-80 overflow-hidden ${roleClass[role]}`}>
          <Show when={role == "assistant"}><img src={assistantAvatar || "/images/role.png"} alt="Avatar" class="w-full h-full object-cover" /></Show>
        </div>
